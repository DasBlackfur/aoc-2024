...
pub fn solve_part1(input: &[Vec<char>]) -> usize {
    ...
    std::thread::Builder::new()
        .stack_size(size_of::<usize>() * 1_000_000)
        .spawn(move || check_tile(&input, 2, &start, 0, &end, &mut positions))
        .unwrap()
        .join()
        .unwrap()
}
    ...
    if score == 0 {
        ...
        if direction != 2 {
            left = check_tile(
                ...
                0,
                &(pos.0 - 1, pos.1),
                score + if direction == 0 { 1 } else { 1001 },
                goal,
                positions,
            );
        }
        if direction != 3 {
            up = check_tile(
                ...
                1,
                &(pos.0, pos.1 - 1),
                score + if direction == 1 { 1 } else { 1001 },
                goal,
                positions,
            );
        }
        if direction != 0 {
            right = check_tile(
                ...
                2,
                &(pos.0 + 1, pos.1),
                score + if direction == 2 { 1 } else { 1001 },
                goal,
                positions,
            );
        }
        if direction != 1 {
            down = check_tile(
                ...
                3,
                &(pos.0, pos.1 + 1),
                score + if direction == 3 { 1 } else { 1001 },
                goal,
                positions,
            );
        }
