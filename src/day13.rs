...
pub fn solve_part1(input: &[Machine]) -> isize {
    input.iter().filter_map(|machine| {
        let x = (-(machine.b_x*machine.prize_y-machine.b_y*machine.prize_x))/(machine.a_x*machine.b_y-machine.a_y*machine.b_x);
        let y = (machine.a_x*machine.prize_y-machine.a_y*machine.prize_x)/(machine.a_x*machine.b_y-machine.a_y*machine.b_x);

        if (x * machine.a_x + y * machine.b_x) == machine.prize_x && (x * machine.a_y + y * machine.b_y) == machine.prize_y {
            return Some(x * 3 + y);
        }
        None
    }).sum()
...
pub fn solve_part2(input: &[Machine]) -> isize {
    input.iter().map(|machine| {
        Machine { prize_x: machine.prize_x + 10000000000000, prize_y: machine.prize_y + 10000000000000, a_x: machine.a_x, a_y: machine.a_y, b_x: machine.b_x, b_y: machine.b_y }
    }).filter_map(|machine| {
        let x = (-(machine.b_x*machine.prize_y-machine.b_y*machine.prize_x))/(machine.a_x*machine.b_y-machine.a_y*machine.b_x);
        let y = (machine.a_x*machine.prize_y-machine.a_y*machine.prize_x)/(machine.a_x*machine.b_y-machine.a_y*machine.b_x);
...
mod tests {
    ...
    #[test]
    fn task_sample() {
        let input = input_generator(r#"Button A: X+94, Y+34
Button B: X+22, Y+67
Prize: X=8400, Y=5400
